...
void setup_tree_view(GtkWidget *treeview)
{
	...
	gtk_tree_view_column_set_title(column, "Friend List");
		...
		GtkTreeViewColumn *column, gpointer data)
{
	...
	if (gtk_tree_model_get_iter(model, &iter, path))
	{
		...
		if (!(haschild = gtk_tree_model_iter_has_child(model, &iter)))
		{
			...
			if (w_chatdlg == NULL)
			{
				if (strcmp(text_split[0], "一班") == 0||strcmp(text_split[0], "二班") == 0||strcmp(text_split[0], "三班") == 0||strcmp(text_split[0], "四班") == 0||strcmp(text_split[0], "Neusoft") == 0) // 群窗口
				{
					// 不允许参与除了自己部门和总群的群聊
					if (my_indices_num != indices_num && indices_num != 0)
					{
						show_info_msg_box(GTK_MESSAGE_INFO, GTK_BUTTONS_OK,
								"you can't talk in other group!");
						return;
					}
					else
